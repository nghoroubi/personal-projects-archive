// Copyright 2021
// Author Nima Ghoroubi. All Rights Reserved.
...
import (
	...
	validator "github.com/ghoroubi/protobuf-validator-go"
)
...
func (p *plugin) Generate(file *generator.FileDescriptor) {
	...
	p.validatorPkg = p.NewImport("github.com/ghoroubi/protobuf-validator-go")
	...
	for _, field := range message.Field {
		...
		if v != nil {
			...
			if &v.Regex != nil && &v.UuidVer != nil {
				_, _ = fmt.Fprintf(os.Stderr, "WARNING: regex and uuid validator is set for field %v.%v, only one of them can be set. Regex and UUID validator is ignored for this field.", ccTypeName, fieldName)
			} else if &v.UuidVer != nil {
				uuid, err := getUUIDRegex(&v.UuidVer)
				if err != nil {
					_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v error %s.\n", ccTypeName, fieldName, err)
				} else {
					v.Regex = uuid
					p.P(`var `,
...
func (p *plugin) generateProto2Message(file *generator.FileDescriptor, message *generator.Descriptor) {
	...
	p.P(`func (v *`, ccTypeName, `) Validate() error {`)
	p.In()
	for _, field := range message.Field {
		...
		if p.validatorWithMessageExists(fieldValidator) {
			_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v is a proto2 message, validator.msg_exists has no effect\n", ccTypeName, fieldName)
		}
		variableName := "v." + fieldName
		...
		if repeated {
			...
			variableName = `v.Get` + fieldName + `()`
		}
		if !repeated && fieldValidator != nil {
			if &fieldValidator.RepeatedCountMin != nil {
				_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v is not repeated, validator.min_elts has no effects\n", ccTypeName, fieldName)
			}
			if &fieldValidator.RepeatedCountMax != nil {
				_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v is not repeated, validator.max_elts has no effects\n", ccTypeName, fieldName)
			}
		}
		if field.IsString() {
...
func (p *plugin) generateProto3Message(file *generator.FileDescriptor, message *generator.Descriptor) {
	...
	for _, field := range message.Field {
		...
		if isOneOf {
			...
			p.P(`if oneOfNester, ok := v.Get` + oneOfName + `().(* ` + oneOfType + `); ok {`)
		...
		if field.IsString() {
			...
			if p.validatorWithMessageExists(fieldValidator) {
				if nullable && !repeated {
					...
					_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v is repeated, validator.msg_exists has no effect\n", ccTypeName, fieldName)
				} else if !nullable {
					_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v is a nullable=false, validator.msg_exists has no effect\n", ccTypeName, fieldName)
				}
			}
			if nullable {
				p.P(`if `, variableName, ` != nil {`)
...
func (p *plugin) generateFloatValidator(variableName string, ccTypeName string, fieldName string, fv *validator.FieldValidator) {
	...
	if &fv.FloatEpsilon != nil && &fv.FloatLt == nil && &fv.FloatGt == nil {
		_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v has no 'float_lt' or 'float_gt' field so setting 'float_epsilon' has no effect.", ccTypeName, fieldName)
	}
	if &fv.FloatLt != nil && &fv.FloatLte != nil {
		_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v has both 'float_lt' and 'float_lte' constraints, only the strictest will be used.", ccTypeName, fieldName)
	...
	if &fv.FloatGt != nil && &fv.FloatGte != nil {
		_, _ = fmt.Fprintf(os.Stderr, "WARNING: field %v.%v has both 'float_gt' and 'float_gte' constraints, only the strictest will be used.", ccTypeName, fieldName)
...
func getUUIDRegex(version *int32) (string, error) {
	if version == nil {
		...
		return fmt.Sprintf(UUIDRegEx, "1-5"), nil
	} else {
		return fmt.Sprintf(UUIDRegEx, strconv.Itoa(int(*version))), nil
	}
}
